...
export class LogicLampsEnvironment {
  ...
  private detailGroups: THREE.Object3D[] = [];
  private detailVisibilityTimer = 0;
  ...
  private createWoodenArchBridge() {
    ...
    this.detailGroups.push(bridgeGroup);
  }
  ...
  private createJapaneseShrinePagoda() {
    ...
    this.detailGroups.push(pagodaGroup);
  }
  ...
  private createVillageHouses() {
    ...
    houseConfigs.forEach((h) => {
      ...
      this.detailGroups.push(houseGroup);
    });
  }
  ...
  private createDistantVillage() {
    ...
    this.detailGroups.push(distantBridge);
    ...
    this.detailGroups.push(distantTorii);
  }
  ...
  private createSakuraAndBamboo() {
    ...
    sakuraPositions.forEach((loc) => {
      ...
      this.detailGroups.push(sakuraTree);
    });
  ...
  private createVillageEntranceToriiGate() {
    ...
    this.detailGroups.push(toriiGroup);
  }
  ...
  private createStationDepartureGate() {
    ...
    this.detailGroups.push(gateGroup);
  }
   ...
   */
  private updateDetailVisibility(delta: number, focusPosition?: THREE.Vector3) {
    if (!focusPosition) return;

    this.detailVisibilityTimer += delta;
    if (this.detailVisibilityTimer < 0.2) return;
    this.detailVisibilityTimer = 0;

    const maxDistance = 34;
    const maxDistanceSquared = maxDistance * maxDistance;
    this.detailGroups.forEach((group) => {
      const dx = group.position.x - focusPosition.x;
      const dz = group.position.z - focusPosition.z;
      group.visible = dx * dx + dz * dz <= maxDistanceSquared;
    });
  }

  public update(delta: number, focusPosition?: THREE.Vector3) {
    const time = performance.now() * 0.001;

    this.updateDetailVisibility(delta, focusPosition);
