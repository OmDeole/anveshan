...
export class PromptifyEnvironment {
  ...
  private buildRichMarketStall(
    ...
    this.detailGroups.push(stallGroup);
  }
  ...
  private createOverheadLanternStrings() {
    ...
    cableZPositions.forEach((zPos, sIdx) => {
      ...
      this.detailGroups.push(cableGroup);
    });
  }
  ...
  private createFlora() {
    ...
    treePositions.forEach((tp) => {
      ...
      this.detailGroups.push(treeGroup);
    });
  }
  ...
  private updateDetailVisibility(delta: number, focusPosition?: THREE.Vector3) {
    ...
    // Keep nearby market detail while removing whole decorative groups from
    // the render traversal once they are well outside the play space.
