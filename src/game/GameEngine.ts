...
export class GameEngine {
  ...
  private onPointerMove = (e: PointerEvent) => {
    ...
    if (this.isPointerDown && this.activePointers.size === 1) {
      ...
      const yawSensitivity = isTouch ? 0.007 : 0.006;
      const pitchSensitivity = isTouch ? 0.005 : 0.004;

      // Rotate camera yaw smoothly
      this.cameraAngleY -= deltaX * yawSensitivity;
      // Adjust camera pitch (elevation angle)
      this.cameraPitch = THREE.MathUtils.clamp(
        this.cameraPitch + deltaY * pitchSensitivity,
